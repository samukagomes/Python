...
class PesquisaSite():
    ...
    def coleta_dados_ali(self):
        ...
        nome=self.navegador.find_elements('xpath', '//*[@class="manhattan--titleText--WccSjUS"]')
        valor=self.navegador.find_elements('xpath', '//*[@class="manhattan--price-sale--1CCSZfK"]')
        ...
        for i in nome:
            try:
                if len(self.prod_ali['nome']) >= 3: break
        ...
        for i in valor:
            try:
                if len(self.prod_ali['valor']) >=3: break
